  ...
  return (
    <div className="space-y-6 text-white">
      <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
        <p className="text-xs uppercase tracking-[0.2em] text-cyan-300/70">Admin Pitches</p>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
        ...
        <form action={createAdminPitch} className="grid gap-2 md:grid-cols-4">
          <input name="name" required placeholder="Nom du terrain" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <select name="tournamentId" required className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm">
            <option value="">Tournoi</option>
            {tournaments.map((tournament) => (
              <option key={tournament.id} value={tournament.id}>{tournament.name}</option>
            ))}
          </select>
          <input name="phaseId" placeholder="Phase ID (optionnel)" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          ...
          {pitches.map((pitch) => (
            <div key={pitch.id} className="flex flex-col gap-2 px-4 py-3">
              <form action={updateAdminPitch} className="grid gap-2 md:grid-cols-6">
                ...
                <input name="name" defaultValue={pitch.name} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
                <select name="tournamentId" defaultValue={pitch.tournamentId} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm">
                  {tournaments.map((tournament) => (
                    <option key={tournament.id} value={tournament.id}>{tournament.name}</option>
                  ))}
                </select>
                <input name="phaseId" defaultValue={pitch.phaseId || ""} placeholder="Phase ID" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
