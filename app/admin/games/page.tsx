  ...
  return (
    <div className="space-y-6 text-white">
      <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
        <p className="text-xs uppercase tracking-[0.2em] text-cyan-300/70">Admin Games</p>
        <h1 className="mt-2 text-2xl md:text-3xl font-black">Catalogue des jeux</h1>
        <Link href="/admin/pitches" className="mt-3 inline-flex text-sm text-cyan-300 hover:text-cyan-200">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
        ...
        <form action={createAdminGame} className="grid gap-2 md:grid-cols-3">
          <input name="name" required placeholder="Nom du jeu" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          ...
          {games.map((game) => (
            <div key={game.id} className="flex flex-col gap-2 px-4 py-3">
              <form action={updateAdminGame} className="grid gap-2 md:grid-cols-5">
                ...
                <input name="name" defaultValue={game.name} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
                <input name="logoUrl" placeholder="Logo URL" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
